...
public:
	...
	int distinctEchoSubstrings(string s) {
		...
		for (int l = 2; l <= n; l += 2) {
			for (int i = 0; i + l <= n; i++) {
				int h1 = gethash(i, i + l / 2 - 1);
				int h2 = gethash2(i, i + l / 2 - 1);
				int h3 = gethash(i + l / 2, i + l - 1);
				int h4 = gethash2(i + l / 2, i + l - 1);
				if (h1==h3 && h2==h4) {
                    int h5= gethash(i,i+l-1);
                    int h6=gethash2(i,i+l-1);
                    long long pro= h5*1LL*h6;
                    m[pro]++;
				}
			}
		}
